...
static void (*library_select_hook)(glui32, glui32) = NULL;
...
glui32 perform_glk(glui32 funcnum, glui32 numargs, glui32 *arglist)
{
  ...
  case 0x0062: /* fileref_create_by_prompt */
    ...
    if (library_select_hook)
      library_select_hook(0x0062, arglist[0]); /*### more args */
  ...
  case 0x00C0: /* select */
    ...
    if (library_select_hook)
      library_select_hook(0x00C0, arglist[0]);
    /* but then fall through to full dispatcher, because there's no real
       need for speed here */
    goto FullDispatcher;
...
void set_library_select_hook(void (*func)(glui32, glui32))
{
